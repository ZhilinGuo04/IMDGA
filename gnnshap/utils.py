...
@torch.no_grad()
def pruned_comp_graph_(node_idx: Union[int, List[int], Tensor],
    num_hops: int,
    edge_index: Tensor,
    neighbor,
    graph,
    ...
    subset = neighbor
    subgraph = graph.subgraph(neighbor.cpu().numpy())
    inv = (subset < node_idx).sum()
    edges = list(subgraph.edges())
    if edges==[]:
        return None,None,None,None
    source_nodes, target_nodes = zip(*edges) 
    new_edge_index = np.array([source_nodes, target_nodes]) 
    new_edge_index = torch.tensor(new_edge_index, dtype=torch.long)
    my_edge_mask = torch.isin(edge_index[0], subset) & torch.isin(edge_index[1], subset)
    my_edge_mask_2 = torch.isin(edge_index[1], subset) & torch.isin(edge_index[0], subset)
    my_edge_mask = my_edge_mask | my_edge_mask_2
